...
def separate_channels(samples, num_channels):
	...
	return [samples[times::num_channels] for times in range(num_channels)]
...
def sine_gen(freq, amplitude, duration_seconds):
	...
	for i in range(int(SAMPLING_FREQ*duration_seconds)):	#for every hz
		# y = A * sin(F * x), où x est en radian, A=amplitude, F=fréquence
...
def convert_to_bytes(samples):
	...
	for sample in samples:
		value = packer.pack(int(sample*MAX_SAMPLE_VALUE))
		data += value
	return data
